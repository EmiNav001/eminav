  ...
  <nav>
    ...
    <ul class="nav-links">
      ...
      <li><a href="#contact">Contact</a></li>
          <li><a href="#" onclick="event.preventDefault();document.getElementById('privacy-policy').style.display='block';document.getElementById('privacy-policy').scrollIntoView()">Privacy</a></li>
          <li><a href="#" onclick="event.preventDefault();document.getElementById('terms-of-service').style.display='block';document.getElementById('terms-of-service').scrollIntoView()">Terms</a></li>
...
Search health facilities by service, capability, and location — so you can make informed decisions with confidence.
  ...
  <section id="find-care" style="min-height:100vh;background:#F7F9F9;padding:100px 5% 120px;display:none">
    <div style="max-width:1200px;margin:0 auto">
      ...
      <!-- DISCLAIMER -->
      <div style="background:linear-gradient(135deg,#FEF3C7 0%,#FDE68A 50%);border:2px solid #F59E0B;border-radius:12px;padding:24px;margin-bottom:35px">
        <div style="font-weight:700;color:#92400E;margin-bottom:10px;display:flex;align-items:center;gap:10px;font-size:1.05rem">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" stroke-width="2.5"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
          Important Disclaimer
        </div>
        <div style="font-size:0.95rem;color:#78350F;line-height:1.7">
      ...
      <div id="intent-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:20px;margin-bottom:60px">
        ...
        <div class="intent-card" onclick="alert('Medical Emergency in ' + document.getElementById('selected-city').textContent)" style="background:linear-gradient(135deg,#FEE2E2 0%,#FECACA 100%);border:2px solid #FCA5A5;border-radius:16px;padding:24px;cursor:pointer;position:relative">
          ...
          <div style="font-size:0.85rem;color:#6B7280">Cardiac, severe pain, breathing issues</div>
        </div>

        <div class="intent-card" onclick="alert('Labor in ' + document.getElementById('selected-city').textContent)" style="background:linear-gradient(135deg,#FCE7F3 0%,#FBCFE8 100%);border:2px solid #F9A8D4;border-radius:16px;padding:24px;cursor:pointer;position:relative">
          ...
          <div style="font-size:0.85rem;color:#6B7280">Contractions, delivery, maternity care</div>
        </div>

        <div class="intent-card" onclick="alert('Child Emergency in ' + document.getElementById('selected-city').textContent)" style="background:linear-gradient(135deg,#FEF3C7 0%,#FDE68A 100%);border:2px solid #FCD34D;border-radius:16px;padding:24px;cursor:pointer;position:relative">
          ...
          <div style="font-size:0.85rem;color:#6B7280">Pediatric emergencies, high fever</div>
        </div>
        ...
        <div class="intent-card" onclick="alert('Dental in ' + document.getElementById('selected-city').textContent)" style="background:linear-gradient(135deg,#FEF3C7 0%,#FDE68A 100%);border:2px solid #FCD34D;border-radius:16px;padding:24px;cursor:pointer;position:relative">
          <div style="position:absolute;top:10px;right:10px;background:#F59E0B;color:white;font-size:0.65rem;font-weight:700;padding:4px 10px;border-radius:100px">URGENT</div>
          <div style="font-size:2.5rem;margin-bottom:12px">🦷</div>
          <div style="font-weight:600;font-size:1.1rem;color:#0A1628;margin-bottom:6px">Dental Emergency</div>
          <div style="font-size:0.85rem;color:#6B7280">Severe toothache, broken tooth</div>
        </div>

        <div class="intent-card" onclick="alert('Ambulance in ' + document.getElementById('selected-city').textContent)" style="background:linear-gradient(135deg,#FEE2E2 0%,#FECACA 100%);border:2px solid #FCA5A5;border-radius:16px;padding:24px;cursor:pointer;position:relative">
          <div style="position:absolute;top:10px;right:10px;background:#DC2626;color:white;font-size:0.65rem;font-weight:700;padding:4px 10px;border-radius:100px">URGENT</div>
          <div style="font-size:2.5rem;margin-bottom:12px">🚨</div>
        ...
        <div class="intent-card" onclick="alert('Pharmacy in ' + document.getElementById('selected-city').textContent)" style="background:white;border:2px solid #E5E7EB;border-radius:16px;padding:24px;cursor:pointer">
          <div style="font-size:2.5rem;margin-bottom:12px">💊</div>
          <div style="font-weight:600;font-size:1.1rem;color:#0A1628;margin-bottom:6px">Need Pharmacy</div>
          <div style="font-size:0.85rem;color:#6B7280">Medications, prescriptions</div>
        </div>

        <div class="intent-card" onclick="alert('Diagnostic in ' + document.getElementById('selected-city').textContent)" style="background:white;border:2px solid #E5E7EB;border-radius:16px;padding:24px;cursor:pointer">
          <div style="font-size:2.5rem;margin-bottom:12px">🔬</div>
          <div style="font-weight:600;font-size:1.1rem;color:#0A1628;margin-bottom:6px">Diagnostic Center</div>
          <div style="font-size:0.85rem;color:#6B7280">X-rays, scans, lab tests</div>
        </div>
        ...
        <div class="intent-card" onclick="alert('Mental Health in ' + document.getElementById('selected-city').textContent)" style="background:white;border:2px solid #E5E7EB;border-radius:16px;padding:24px;cursor:pointer">
          ...
          <div style="font-size:0.85rem;color:#6B7280">Therapy, counseling, support</div>
        </div>
...
EmiNav was built because finding the right healthcare in African cities shouldn’t require luck, word-of-mouth, insider knowledge, or wasted minutes spent on endlessly searching.
  ...
  <footer>
    ...
    <div class="footer-bottom">
      <div class="footer-bottom-text" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;font-size:0.8rem">
        ...
        <a href="#" onclick="event.preventDefault();document.getElementById('privacy-policy').style.display='block';document.getElementById('privacy-policy').scrollIntoView({behavior:'smooth'})" style="color:rgba(255,255,255,0.5);text-decoration:none">Privacy</a>
        <span style="color:rgba(255,255,255,0.2)">•</span>
        <a href="#" onclick="event.preventDefault();document.getElementById('terms-of-service').style.display='block';document.getElementById('terms-of-service').scrollIntoView({behavior:'smooth'})" style="color:rgba(255,255,255,0.5);text-decoration:none">Terms</a>
      </div>
  ...
  <!-- PRIVACY POLICY PAGE -->
  <div id="privacy-policy" style="display:none;min-height:100vh;background:#F7F9F9;padding:120px 5% 80px">
    <div style="max-width:800px;margin:0 auto;background:white;padding:60px;border-radius:16px;box-shadow:0 4px 6px rgba(0,0,0,0.05)">
      <button onclick="document.getElementById('privacy-policy').style.display='none';window.scrollTo({top:0,behavior:'smooth'})" style="background:#0D6E6E;color:white;border:none;padding:10px 24px;border-radius:8px;cursor:pointer;margin-bottom:30px;font-family:'DM Sans',sans-serif;font-weight:600">← Back to Home</button>
  ...
  <div id="terms-of-service" style="display:none;min-height:100vh;background:#F7F9F9;padding:120px 5% 80px">
    <div style="max-width:800px;margin:0 auto;background:white;padding:60px;border-radius:16px;box-shadow:0 4px 6px rgba(0,0,0,0.05)">
      <button onclick="document.getElementById('terms-of-service').style.display='none';window.scrollTo({top:0,behavior:'smooth'})" style="background:#0D6E6E;color:white;border:none;padding:10px 24px;border-radius:8px;cursor:pointer;margin-bottom:30px;font-family:'DM Sans',sans-serif;font-weight:600">← Back to Home</button>
